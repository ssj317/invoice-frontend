                ...
                <div key={pageIndex} className="page-container" style={{ width: '210mm', height: '297mm', position: 'relative', margin: '0 auto', backgroundColor: 'white', lineHeight: 'normal', fontSize: '14px', overflow: 'hidden', marginBottom: pageIndex < pages.length - 1 ? '20px' : '0' }}>
                    ...
                    {page.type === 'page2' && (
                        <div className="px-10 py-4 text-sm" style={{ paddingTop: '16px' }}>
                          ...
                          {'supportItems' in page && (page.supportItems as string[])?.length > 0 && (
                                <div className="mb-3">
                                    <p className="font-bold mb-1">{data.supportTitle}</p>
                                    <ul className="ml-4 space-y-1">
                                      {'supportItems' in page && (page.supportItems as string[]).map((t: string, i: number) => (
                                            <li key={i} className="flex gap-2"><span>•</span><span>{t}</span></li>
                                        ))}
                                    </ul>
                                </div>
                            )}
                         {'ipItems' in page && (page.ipItems as string[])?.length > 0 && (
                                <div className="mb-3">
                                    <p className="font-bold mb-1">{data.ipTitle}</p>
                                    <ul className="ml-4 space-y-1">
                                        {'ipItems' in page && (page.ipItems as string[]).map((t: string, i: number) => (
                                            <li key={i} className="flex gap-2"><span>•</span><span>{t}</span></li>
                                        ))}
                                    </ul>
                                </div>
                            )}
                       {'showClosing' in page && (page.showClosing as boolean) && (
                                <>
                                    <div className="mb-3">
                                        <p className="font-bold mb-1">{data.confidentialityTitle}</p>
                                        <ul className="ml-4">
                    ...
                    {page.type === 'support' && (
                        <div className="px-10 py-6 text-sm" style={{ paddingTop: '40px' }}>
                          {'showSupportHeader' in page && (page.showSupportHeader as boolean) && (
                                <p className="font-bold mb-2">{data.supportTitle}</p>
                            )}
                            <ul className="ml-4 space-y-1">
                                {'supportItems' in page && (page.supportItems as string[]).map((t: string, i: number) => (
                    ...
                    {page.type === 'ip' && (
                        <div className="px-10 py-6 text-sm" style={{ paddingTop: '40px' }}>
                          {'showIpHeader' in page && (page.showIpHeader as boolean) && (
                                <p className="font-bold mb-2">{data.ipTitle}</p>
                            )}
                            <ul className="ml-4 space-y-1 mb-5">
                               {'ipItems' in page && (page.ipItems as string[]).map((t: string, i: number) => (
