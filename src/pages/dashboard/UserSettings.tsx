...
import { useState, useEffect } from 'react';
import { Camera, ChevronDown, Save, Loader } from 'lucide-react';
import DashboardLayout from './DashboardLayout';
import { useNavigate } from 'react-router-dom';
                    ...
                    <div className="flex-1 min-w-0">
                                    ...
                                    <>
                                        ...
                                        <div className="mb-6 sm:mb-8">
                                            ...
                                            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4">
                                                <div className="relative flex-shrink-0">
                                                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-2xl sm:text-3xl font-bold">
                                                        {personalData.fullName ? personalData.fullName.charAt(0).toUpperCase() : 'U'}
                                                    </div>
                                                    <button className="absolute bottom-0 right-0 w-7 h-7 sm:w-8 sm:h-8 bg-white rounded-full border-2 border-gray-200 flex items-center justify-center hover:bg-gray-50">
                                                        <Camera className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-600" />
                                                    </button>
                                                ...
                                                <div className="text-center sm:text-left">
                                                    <h4 className="text-base sm:text-lg font-bold text-gray-900">{personalData.fullName || 'Your Name'}</h4>
                                                    <p className="text-xs sm:text-sm text-gray-500">{personalData.email}</p>
                                                    {personalData.companyName && <p className="text-xs sm:text-sm text-gray-400">{personalData.companyName}</p>}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                                            ...
                                            <div>
                                                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Email</label>
                                                <input type="email" value={personalData.email} readOnly
                                                    className="w-full px-3 sm:px-4 py-2 text-sm border border-gray-300 rounded-lg bg-gray-50 text-gray-500 cursor-not-allowed" />
                                            </div>
                                            <div>
                                        ...
                                        {saveMsg && (
                                            <p className={`text-sm mb-3 ${saveMsg.includes('Failed') ? 'text-red-600' : 'text-green-600'}`}>{saveMsg}</p>
                                        )}
                        ...
                        {activeTab === 'notifications' && (
                            <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 md:p-8">
                                ...
                                <div className="space-y-4 sm:space-y-6">
                                    {[
                                        { key: 'general', label: 'General Notifications', desc: 'Get updates & alerts regarding general mails' },
                                        { key: 'accounting', label: 'Accounting Notifications', desc: 'Get updates & alerts regarding accounting emails' },
                                        { key: 'leads', label: 'Leads Notifications', desc: 'Get updates & alerts regarding lead mails' },
                                        { key: 'projects', label: 'Project & Escrow Notifications', desc: 'Get important updates on projects including milestones & payments' },
                                        { key: 'payments', label: 'Payments & Verification Notifications', desc: 'Get important updates on payments and verification' },
                                    ].map(({ key, label, desc }, i, arr) => (
                                        <div key={key} className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 py-3 sm:py-4 ${i < arr.length - 1 ? 'border-b border-gray-200' : ''}`}>
                                            <div className="flex-1 min-w-0">
                                                <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1">{label}</h3>
                        ...
                        {activeTab === 'security' && (
                            <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 md:p-8">
                                ...
                                <div className="space-y-4 sm:space-y-6">
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 py-3 sm:py-4 border-b border-gray-200">
                                        <div className="flex-1">
                                            <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1">Authentication Method</h3>
                                            <p className="text-xs sm:text-sm text-gray-600">Your account uses OTP-based login via email — no password required</p>
                                        </div>
                                        <span className="px-3 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full whitespace-nowrap">OTP Enabled</span>
                                    </div>
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 py-3 sm:py-4">
                                        <div className="flex-1">
                                            <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1">Logout from all devices</h3>
                                            <p className="text-xs sm:text-sm text-gray-600">This will log you out from all devices</p>
                                        </div>
