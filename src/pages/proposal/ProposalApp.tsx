...
const ProposalApp = () => {
    ...
    const getDocumentData = () => {
        switch (templateType) {
            case 'service-proposal': return invoiceState.proposalData;
            case 'service-agreement': return invoiceState.agreementData;
            case 'tax-invoice': return invoiceState.taxInvoiceData;
            default: return null;
        }
    };
            ...
            if (templateType === 'tax-invoice') {
                ...
                await html2pdf()
                    .set({
                        ...
                        html2canvas: { scale: 2, useCORS: true, logging: false, letterRendering: true, allowTaint: true, height: 2245, windowHeight: 2245 },
    ...
    const handleSendEmail = async () => {
        if (!emailTo) { setEmailMsg('Please enter recipient email.'); return; }
        if (!savedDocId) { setEmailMsg('Please save the document first before emailing.'); return; }
        ...
        try {
            ...
            setTimeout(() => { setShowEmailModal(false); setEmailMsg(''); }, 2000);
        } catch (err: any) {
    ...
    const renderTemplate = () => {
        if (activeStep === 2) {
            switch (templateType) {
                case 'service-proposal': return <div ref={previewRef}><ServiceProposalPreview /></div>;
                case 'service-agreement': return <div ref={previewRef}><ServiceAgreementPreview /></div>;
                case 'tax-invoice': return <div ref={previewRef}><TaxInvoicePreview /></div>;
                default: return <div>Template not found</div>;
            }
        } else {
            switch (templateType) {
                case 'service-proposal': return <ServiceProposal />;
                case 'service-agreement': return <ServiceAgreement />;
                case 'tax-invoice': return <TaxInvoice />;
                default: return <div>Template not found</div>;
            }
        }
    };

    const docTitle = templateType === 'service-proposal' ? 'Service Proposal'
        : templateType === 'service-agreement' ? 'Service Agreement'
        : 'Tax Invoice';
    ...
    return (
        <div>
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4">
                ...
                {saveMsg && (
                    <div className={`max-w-7xl mx-auto mt-2 text-sm font-medium ${saveMsg.includes('success') ? 'text-green-600' : 'text-red-500'}`}>
                        {saveMsg}
                    </div>
                )}
            </div>
            ...
            {showEmailModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
                        ...
                        {emailMsg && (
                            <p className={`mt-3 text-sm font-medium ${emailMsg.includes('success') ? 'text-green-600' : 'text-red-500'}`}>
                                {emailMsg}
                            </p>
                        )}
